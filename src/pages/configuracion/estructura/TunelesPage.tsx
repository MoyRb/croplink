...
export function TunelesPage() {
  ...
  return (
    <CrudShell title="Túneles" searchPlaceholder="Buscar túnel" query={query} setQuery={setQuery} onNew={() => { setForm({ id: '', sectorId: '', name: '', description: '', code: '' }); setModalOpen(true) }} rows={rows} toastMessage={feedback.toastMessage} errorMessage={feedback.errorMessage || loadError} isLoading={isLoading} emptyMessage="No hay túneles registrados."
      renderRow={(item) => (
    ...
    >
      <Modal open={modalOpen} title={form.id ? 'Editar túnel' : 'Nuevo túnel'} onClose={() => setModalOpen(false)}>
        <form onSubmit={stopSubmit(() => feedback.run(async () => { if (!organizationId) throw new Error('Perfil sin organización asignada.'); await upsertTunnelSupabase(organizationId, form); await reload(); setModalOpen(false) }, 'Túnel guardado.'))} className="space-y-2">
