...
import { deleteCropSupabase, deleteRanchCropSeasonSupabase, upsertCropSupabase, upsertRanchCropSeasonSupabase } from '../../../lib/operationCatalog/supabaseRepo'
...
export function CultivosPage() {
  ...
  return (
    <div className="space-y-4">
      <CrudShell title="Cultivos" searchPlaceholder="Buscar cultivo" query={query} setQuery={setQuery} onNew={() => { setForm({ id: '', name: '', description: '' }); setModalOpen(true) }} rows={rows} toastMessage={feedback.toastMessage} errorMessage={feedback.errorMessage || loadError} isLoading={isLoading} emptyMessage="No hay cultivos registrados."
        renderRow={(item) => (
      ...
      >
        <Modal open={modalOpen} title={form.id ? 'Editar cultivo' : 'Nuevo cultivo'} onClose={() => setModalOpen(false)}>
          <form onSubmit={stopSubmit(() => feedback.run(async () => { if (!organizationId) throw new Error('Perfil sin organización asignada.'); await upsertCropSupabase(organizationId, form); await reload(); setModalOpen(false) }, 'Cultivo guardado.'))} className="space-y-2">
        ...
        <DeleteModal open={Boolean(deleteId)} onClose={() => setDeleteId('')} onConfirm={() => feedback.run(async () => { await deleteCropSupabase(deleteId); await reload(); setDeleteId('') }, 'Cultivo eliminado.')} />
      </CrudShell>
