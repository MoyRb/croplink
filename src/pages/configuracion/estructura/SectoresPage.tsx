...
export function SectoresPage() {
  ...
  const [form, setForm] = useState({ id: '', ranchId: '', number: '', description: '', areaHa: '' })
  ...
  return (
    <CrudShell title="Sectores" searchPlaceholder="Buscar sector" query={query} setQuery={setQuery} onNew={() => { setForm({ id: '', ranchId: '', number: '', description: '', areaHa: '' }); setModalOpen(true) }} rows={rows} toastMessage={feedback.toastMessage} errorMessage={feedback.errorMessage || loadError} isLoading={isLoading} emptyMessage="No hay sectores registrados."
      renderRow={(item) => (
        <div key={item.id} className="flex items-center justify-between rounded-xl border border-[#E5E7EB] p-3">
          ...
          <div className="flex gap-2"><Button variant="secondary" onClick={() => { setForm({ id: item.id, ranchId: item.ranchId, number: item.number != null ? String(item.number) : '', description: item.description || '', areaHa: item.areaHa != null ? String(item.areaHa) : '' }); setModalOpen(true) }}>Editar</Button><Button variant="secondary" onClick={() => setDeleteId(item.id)}>Eliminar</Button></div>
        </div>
      )}
    >
      <Modal open={modalOpen} title={form.id ? 'Editar sector' : 'Nuevo sector'} onClose={() => setModalOpen(false)}>
        <form onSubmit={stopSubmit(() => feedback.run(async () => {
          ...
          await upsertSectorSupabase(organizationId, { id: form.id, ranchId: form.ranchId, name: '', number: numberVal, description: form.description, areaHa: areaHaRaw !== '' ? Number(areaHaRaw) : null })
