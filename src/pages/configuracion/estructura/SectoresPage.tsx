...
export function SectoresPage() {
  ...
  return (
    <CrudShell title="Sectores" searchPlaceholder="Buscar sector" query={query} setQuery={setQuery} onNew={() => { setForm({ id: '', ranchId: '', name: '', description: '', code: '' }); setModalOpen(true) }} rows={rows} toastMessage={feedback.toastMessage} errorMessage={feedback.errorMessage || loadError} isLoading={isLoading} emptyMessage="No hay sectores registrados."
      renderRow={(item) => (
    ...
    >
      <Modal open={modalOpen} title={form.id ? 'Editar sector' : 'Nuevo sector'} onClose={() => setModalOpen(false)}>
        <form onSubmit={stopSubmit(() => feedback.run(async () => { if (!organizationId) throw new Error('Perfil sin organización asignada.'); await upsertSectorSupabase(organizationId, form); await reload(); setModalOpen(false) }, 'Sector guardado.'))} className="space-y-2">
